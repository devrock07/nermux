...
public class TermuxTerminalSessionActivityClient extends TermuxTerminalSessionClientBase {
    ...
    public void requestCloseSession(final TerminalSession sessionToClose) {
        if (sessionToClose == null) return;
        ...
        if (sessionToClose.isRunning()) {
            new AlertDialog.Builder(mActivity)
                ...
                .setPositiveButton(android.R.string.yes, (dialog, id) -> {
                    dialog.dismiss();
                    closeSessionNow(sessionToClose);
                })
    ...
    private void closeSessionNow(TerminalSession sessionToClose) {
        TermuxService service = mActivity.getTermuxService();
        if (service == null || sessionToClose == null) return;

        int index = service.getIndexOfSession(sessionToClose);
        if (index < 0) return;

        int size = service.getTermuxSessionsSize();
        TerminalSession replacementSession = getReplacementSession(service, index, size);
        boolean closingCurrentSession = sessionToClose == mActivity.getCurrentSession();

        if (sessionToClose.isRunning()) {
            if (closingCurrentSession && replacementSession != null)
                setCurrentSession(replacementSession);

            TermuxSession termuxSession = service.getTermuxSessionForTerminalSession(sessionToClose);
            if (termuxSession != null)
            ...
            removeFinishedSession(sessionToClose);
        }
    }

    @Nullable
    private TerminalSession getReplacementSession(TermuxService service, int index, int size) {
        if (service == null || size <= 1) return null;

        int replacementIndex = index >= size - 1 ? index - 1 : index + 1;
        TermuxSession replacementTermuxSession = service.getTermuxSession(replacementIndex);
        return replacementTermuxSession == null ? null : replacementTermuxSession.getTerminalSession();
    }
